...
filesystem outside the mounted workspace, reaching the network with
`--network none` set, escaping the container's resource limits, or
escalating privileges inside or out of the container — please report it
...
- Anything that lets a command reach the network with
  `BROKKR_SANDBOX_NETWORK=none` set.
